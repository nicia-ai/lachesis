...
const root = await mkdtemp(resolve(tmpdir(), "lachesis-stage5-packed-"));
...
const schema = {
  id: "packed-stage5/item", version: "1", description: "Packed item.",
...
const result = createCatalog({
  identity: { id: "packed-stage5/catalog", version: "1" },
...
`,
  ...
  if (
    !verification.includes('"command":{"commandIdentity"') ||
    !verification.includes('"id":"report.verify"') ||
    !verification.includes('"outcomeExitCode":0')
  )
    throw new Error("Packed detached report verification did not pass.");
  const [lockfile, sourceReport, manifest] = await Promise.all([
    readFile(resolve(root, "pnpm-lock.yaml"), "utf8"),
    readFile(resolve(root, "source-report.json"), "utf8"),
    readFile(resolve(root, "manifest.json"), "utf8"),
  ]);
  if (
    lockfile.includes("workspace:") ||
    !sourceReport.includes('"id":"catalog.manifest"') ||
    !manifest.includes('"formatVersion":"1"')
  )
    throw new Error("Packed consumer artifact boundary is invalid.");
  process.stdout.write(
    `${JSON.stringify({
      protocol: "lachesis-m8b1-stage5-packed-consumer/1",
      node: process.version,
      typescript: "6.0.3",
      skipLibCheck: false,
      workspaceImports: false,
      detachedVerification: true,
      providerCalls: 0,
      status: "pass",
    })}\n`,
  );
