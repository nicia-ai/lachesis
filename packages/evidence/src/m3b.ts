...
function evidenceValueDerivations(
  facts: ReadonlyArray<z.infer<typeof evidenceFactSchema>>,
  contract: Extract<M3bAnswerContract, Readonly<{ role: "evidence-values" }>>,
): ReadonlyArray<M3bVisibleDerivation> {
  let selections: ReadonlyArray<
    ReadonlyArray<z.infer<typeof evidenceFactSchema>>
  > = [[]];
  ...
  const unique = new Map<string, M3bVisibleDerivation>();
